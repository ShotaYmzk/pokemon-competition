...
Self-contained (no imports from sibling agent/*.py or explore/*.py) because
Kaggle's submission tarball ships agent/ as a flat, non-package directory
executed via exec(); cross-file imports between sibling scripts are not
reliable under that execution model (same constraint documented in
agent/mcts.py).

Policy: heuristic greedy agent (lethal attack > max damage > energy
attachment toward an attack cost > play active > random fallback), wrapped
in a hard exception/timeout guard so a crash or stall never costs the match
(a crash or timeout = an automatic loss per competition rules).
"""
import os
import sys
import json
...
def _load_deck(config=None):
    """Load the 60-card submission deck (card IDs from JP_Card_Data.csv)."""
    global _deck
...
def _greedy_decision(obs, select, config):
    """Pick options for the current select using the greedy heuristic.

    Returns a list of chosen option indices, or None if the greedy logic
    doesn't apply to this select (caller should use the random fallback).
    """
    ...
    energy_attach_choices = []  # (option_idx, target_is_active, energy_card)
    ...
    # 4) Else option that puts a Pokemon into the active slot (could set up a future KO).
    play_active_choices = [
...
def agent(obs, config=None):
    """Kaggle entry point. Deck-select phase returns the 60-card deck;
    every normal turn is decided by the greedy heuristic, with a hard
    exception/timeout guard so a crash or stall never costs the match
    outright (falls back to a random legal pick instead)."""
    ...
    try:
        if time.perf_counter() - t0 > DECISION_TIMEOUT_SECONDS:
            return _random_fallback(select)
        choice = _greedy_decision(obs, select, config)
        if time.perf_counter() - t0 > DECISION_TIMEOUT_SECONDS:
            return _random_fallback(select)
