...
def search_step(agent_ptr, search_id, action, select=None):
    """Call SearchStep with option-array positions.

    SearchStep uses the same encoding as battle_select: pass the selected
    option indices as one int array, including multi-select prompts. The
    returned state contains the next ``searchId``; callers that perform a
    rollout must pass that id into the next SearchStep call.
    """
